...
["dateDay","likhtan","venue","mapLink"].forEach(id=>{
...
return t
.replace("{{NAME}}",g?.name||"મનસુખભાઈ")
.replace("{{DATE_DAY}}",dateDay.value)
.replace("{{INVITATION_TYPE}}",g?.type||"સહપરિવાર ને")
.replace("{{LIKHTAN}}",likhtan.value)
.replace("{{VENUE}}",venue.value)
.replace("{{MAP_LINK}}",mapLink.value);
}

function updatePreview(){
const g=guests[0];
const txt=applyVars(templateBox.value,g);
document.getElementById("preview").textContent=txt;
}

function generate(){
const table=document.getElementById("table");
table.innerHTML="<tr><th>#</th><th>Name</th><th>Status</th><th>Send</th></tr>";
guests.forEach((g,i)=>{
table.innerHTML+=`
<tr id="row${i}">
<td>${i+1}</td>
...
function send(i){
const g=guests[i];
const msg=applyVars(templateBox.value,g);
