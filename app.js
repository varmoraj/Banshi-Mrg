...
function addManual() {
  ...
  if (!name || number.length !== 10) {
    alert("Valid Name અને 10 digit Mobile નાખો");
    return;
  }
...
<td><button onclick="sendInvite(${i})">Send</button></td>
</tr>`;
  });
...
function sendInvite(i) {
  ...
  window.open(
    "https://wa.me/91" + g.number + "?text=" + encodeURIComponent(msg),
    "_blank"
  );
  g.sent = true;
  render();
...
function updatePreview() {
  if (guests.length > 0)
    ...
    preview.textContent = templateBox.value;
}

/* Stats */
function updateStats() {
  stats.innerText =
    `Total: ${guests.length} | Sent: ${guests.filter(g => g.sent).length}`;
}
